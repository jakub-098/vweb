...
export async function POST(request: Request) {
  try {
    ...
    // Map ids 1,2,3 to the respective columns.
    const column = id === 1 ? "config" : id === 2 ? "upload" : "purchase";
    ...
    await pool.query(
      `INSERT INTO analytics (id, config, upload, purchase)
       VALUES (DATE_FORMAT(CURDATE(), '%Y%m%d'), 0, 0, 0)
