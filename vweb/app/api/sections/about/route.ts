...
export async function POST(request: Request) {
  try {
    ...
    try {
      const [rows] = await pool.query<any[]>(
        "SELECT image_name, user_email FROM section_about WHERE order_id = ? ORDER BY id DESC LIMIT 1",
        [orderId]
      );
      if (Array.isArray(rows) && rows.length > 0) {
      ...
      console.error("Failed to load previous section_about image", lookupError);
    }
    ...
    if (imageFile) {
      ...
      if (previousImageName && previousImageName.trim().length > 0) {
        try {
          ...
          console.error("Failed to delete previous section_about image", cleanupError);
        }
      }
...
export async function DELETE(request: Request) {
  try {
    ...
    const [rows] = await pool.query<any[]>(
      "SELECT id, image_name, user_email FROM section_about WHERE order_id = ? ORDER BY id DESC LIMIT 1",
      [orderId],
    );
    ...
    if (imageName && imageName.trim().length > 0 && email) {
      try {
        ...
        console.error("Failed to delete section_about image from storage", err);
      }
    }

    await pool.execute<ResultSetHeader>(
      "UPDATE section_about SET image_name = '' WHERE id = ?",
      [id],
    );

    return NextResponse.json({ success: true, found: true });
  } catch (error) {
    console.error("Error deleting section_about image", error);
    return NextResponse.json(
